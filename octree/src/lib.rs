...
use collision::{Aabb, Aabb2, Contains, Discrete};
use oct_helper::*;
use quad_helper::*;
use std::mem;
...
macro_rules! custom_dimension {
    ($PointN:ident{ $($p_field:ident),+ }, $VectorN:ident{ $($v_field:ident),+ }, $AabbN:ident, $Helper:ident ,$d:expr) => {
        ...
        #[inline]
        pub fn intersects<S: BaseFloat>(a: &$AabbN<S>, b: &$AabbN<S>) -> bool {
            a.intersects(b)
        }

        /// aabb的查询函数的参数
        pub struct AbQueryArgs<S: BaseFloat, T> {
            aabb: $AabbN<S>,
            result: Vec<(usize, T)>,
        }
        impl<S: BaseFloat, T: Clone> AbQueryArgs<S, T> {
        ...
        pub fn ab_query_func<S: BaseFloat, T: Clone>(
        ...
        impl<S: BaseFloat, T> Tree<S, T> {
            ...
            pub fn shift(&mut self, id: usize, distance: $VectorN<S>) -> bool {
                let r = match self.ab_map.get_mut(id) {
                    Some(node) => {
                        node.aabb = $AabbN::new(node.aabb.min + distance, node.aabb.max + distance);
                        update(
                            &mut self.branch_slab,
                            &self.adjust,
                            self.deep,
        ...
        // 计算该aabb对应的层
        #[inline]
        fn calc_layer<S: BaseFloat>(loose: &$VectorN<S>, el: &$VectorN<S>) -> usize {
            $Helper::calc_layer(loose, el)
        }
        // 判断所在的子节点
        #[inline]
        fn get_contain_child<S: BaseFloat, T>(parent: &BranchNode<S>, node: &AbNode<S, T>) -> usize {
            $Helper::get_contain_child(&parent.aabb, &parent.loose, &node.aabb)
        }
        // 判断所在的子节点
        #[inline]
        fn get_child<S: BaseFloat, T>(point: $PointN<S>, node: &AbNode<S, T>) -> usize {
            $Helper::get_child(point, &node.aabb)
        }
        // ab节点下降
        fn down<S: BaseFloat, T>(
        ...
        // 从NodeList中移除，并可能添加
        pub fn remove_add<S: BaseFloat, T>(
            tree: &mut Tree<S, T>,
            id: usize,
        ...
        // 修复prev
        #[inline]
        fn fix_prev<S: BaseFloat, T>(map: &mut VecMap<AbNode<S, T>>, mut head: usize) {
            if head == 0 {
                return;
            }
                    ...
                    match node.childs[$i] {
                        ChildNode::Branch(branch, ref num) if *num > 0 => {
                            if branch_func(branch_arg, &$a) {
                                query(branch_slab, ab_map, branch, branch_arg, branch_func, ab_arg, ab_func);
                            }
                        }
        ...
        // 和指定的列表进行碰撞
        fn collision_list<S: BaseFloat, T, A>(
...
// 和指定的节点进行碰撞
// fn collision_node<S: BaseFloat, T, A>(
//   branch_slab: &Slab<BranchNode<S>>,
...
custom_dimension!(Point2 { x, y }, Vector2 { x, y }, Aabb2, QuadHelper, 4);
#[test]
fn test1() {
    println!("test1-----------------------------------------");
    ...
    tree.update(
        1,
        Aabb2::new(Point2::new(0.0, 0.0), Point2::new(1000.0, 700.0)),
    );
    for i in 1..tree.ab_map.len() + 1 {
        println!("00000, id:{}, ab: {:?}", i, tree.ab_map.get(i).unwrap());
    }
    ...
    tree.update(
        4,
        Aabb2::new(Point2::new(0.0, 700.0), Point2::new(1000.0, 1400.0)),
    );

    tree.update(
        5,
        Aabb2::new(Point2::new(0.0, 1400.0), Point2::new(1000.0, 1470.0)),
    );
    tree.update(
        6,
        Aabb2::new(Point2::new(0.0, 1470.0), Point2::new(1000.0, 1540.0)),
    );
    tree.update(
        1,
        Aabb2::new(Point2::new(0.0, 0.0), Point2::new(1000.0, 700.0)),
    ...
    //   tree.update(1, Aabb2::new(Point2::new(0.0,0.0,0.0), Point2::new(1000.0, 800.0, 1.0)));
    //   tree.update(2, Aabb2::new(Point2::new(0.0,0.0,0.0), Point2::new(1000.0, 800.0, 1.0)));
    //   tree.update(3, Aabb2::new(Point2::new(0.0,0.0,0.0), Point2::new(1000.0, 800.0, 1.0)));
    //   tree.update(4, Aabb2::new(Point2::new(0.0,0.0,0.0), Point2::new(1000.0, 800.0, 1.0)));

    //   tree.update(5, Aabb2::new(Point2::new(0.0,800.0,0.0), Point2::new(1000.0, 1600.0, 1.0)));

    //    tree.update(6, Aabb2::new(Point2::new(0.0,1600.0,0.0), Point2::new(1000.0, 2400.0, 1.0)));
    //   tree.update(7, Aabb2::new(Point2::new(0.0,2400.0,0.0), Point2::new(1000.0, 3200.0, 1.0)));
    //   for i in 1..tree.ab_map.len() + 1 {
    ...
    let aabb = Aabb2::new(Point2::new(500f32, 500f32), Point2::new(500f32, 500f32));
...
fn test2() {
    ...
    tree.update(1, Aabb2::new(Point2::new(0.0, 0.0), Point2::new(0.0, 0.0)));
    tree.collect();
    for i in 1..tree.branch_slab.len() + 1 {
        println!(
            "000000 000000, id:{}, branch: {:?}",
    ...
    for i in 0..6 {
        tree.add(
            i + 1,
            Aabb2::new(Point2::new(0.0, 0.0), Point2::new(0.1, 0.1)),
            i + 1,
        );
    }
    for i in 1..tree.branch_slab.len() + 1 {
    ...
    println!("outer:{:?}", tree.outer);
    let aabb = Aabb2::new(Point2::new(0.05f32, 0.05f32), Point2::new(0.05f32, 0.05f32));
    let mut args: AbQueryArgs<f32, usize> = AbQueryArgs::new(aabb.clone());
    tree.query(&aabb, intersects, &mut args, ab_query_func);
...
#[test]
fn test3() {
    let aabb = Aabb2::new(Point2::new(700.0, 100.0), Point2::new(700.0, 100.0));
    let mut args: AbQueryArgs<f32, usize> = AbQueryArgs::new(aabb.clone());

    // let mut tree = Tree::new(Aabb2::new(Point2::new(0f32,0f32,0f32), Point2::new(1000f32,1000f32,1000f32)),
    // 	0,
    ...
    tree.update(
        0,
        Aabb2::new(Point2::new(0.0, 0.0), Point2::new(100.0, 700.0)),
    );
    tree.update(
        1,
        Aabb2::new(Point2::new(0.0, 0.0), Point2::new(100.0, 700.0)),
    ...
    tree.update(
        2,
        Aabb2::new(Point2::new(600.0, 600.0), Point2::new(640.0, 640.0)),
    );
    ...
    tree.add(
        9,
        Aabb2::new(Point2::new(0.0, 0.0), Point2::new(40.0, 40.0)),
        1,
    );
    tree.collect();
    for i in 1..tree.branch_slab.len() + 1 {
        println!("test||||||, id:{}, branch: {:?}", i, tree.branch_slab.get(i));
    }
    for i in 1..tree.ab_map.len() + 1 {
    ...
    tree.update(
        3,
        Aabb2::new(Point2::new(0.0, 0.0), Point2::new(100.0, 350.0)),
    );
    tree.update(
        4,
        Aabb2::new(Point2::new(0.0, 0.0), Point2::new(80.0, 175.0)),
    );
    tree.update(
        5,
        Aabb2::new(Point2::new(0.0, 0.0), Point2::new(64.0, 140.0)),
    );
    tree.update(
        6,
        Aabb2::new(Point2::new(0.0, 0.0), Point2::new(512.0, 112.0)),
    );
    tree.update(
        7,
        Aabb2::new(Point2::new(0.0, 0.0), Point2::new(410.0, 90.0)),
    );
    tree.update(
        8,
        Aabb2::new(Point2::new(800.0, 0.0), Point2::new(160.0, 175.0)),
    );
    tree.update(
        9,
        Aabb2::new(Point2::new(800.0, 0.0), Point2::new(144.0, 140.0)),
    );
    tree.update(
        1,
        Aabb2::new(Point2::new(0.0, 0.0), Point2::new(1000.0, 700.0)),
    ...
    tree.update(
        5,
        Aabb2::new(Point2::new(0.0, 0.0), Point2::new(640.0, 140.0)),
    );
    tree.update(
        6,
        Aabb2::new(Point2::new(0.0, 0.0), Point2::new(512.0, 112.0)),
    ...
    tree.update(
        9,
        Aabb2::new(Point2::new(800.0, 0.0), Point2::new(140.0, 140.0)),
    );
    for i in 1..tree.branch_slab.len() + 1 {
        println!("test||||||, id:{}, branch: {:?}", i, tree.branch_slab.get(i));
    }
    ...
    tree.update(
        3,
        Aabb2::new(Point2::new(0.0, 0.0), Point2::new(1000.0, 350.0)),
    );
    tree.update(
        4,
        Aabb2::new(Point2::new(0.0, 0.0), Point2::new(800.0, 175.0)),
    );
    tree.update(
        8,
        Aabb2::new(Point2::new(800.0, 0.0), Point2::new(1600.0, 175.0)),
    );
    tree.update(
        9,
        Aabb2::new(Point2::new(800.0, 0.0), Point2::new(1440.0, 140.0)),
    );
    tree.query(&aabb, intersects, &mut args, ab_query_func);
    for i in 1..tree.branch_slab.len() + 1 {
        println!("test||||||, id:{}, branch: {:?}", i, tree.branch_slab.get(i));
...
//     tree.add(
//         1,
//         Aabb2::new(Point2::new(0.0, 0.0, 0.0), Point2::new(1.0, 1.0, 1.0)),
//         1,
//     );
//     tree.add(
//         2,
...
//     tree.update(
//         0,
//         Aabb2::new(Point2::new(0.0, 0.0, 0.0), Point2::new(1000.0, 700.0, 1.0)),
//     );
//     tree.update(
//         1,
//         Aabb2::new(Point2::new(0.0, 0.0, 0.0), Point2::new(1000.0, 700.0, 1.0)),
...
//     tree.update(
//         3,
//         Aabb2::new(Point2::new(0.0, 0.0, 0.0), Point2::new(1000.0, 350.0, 1.0)),
//     );
//     tree.update(
//         4,
//         Aabb2::new(Point2::new(0.0, 0.0, 0.0), Point2::new(800.0, 175.0, 1.0)),
//     );
//     tree.update(
//         5,
//         Aabb2::new(Point2::new(0.0, 0.0, 0.0), Point2::new(640.0, 140.0, 1.0)),
//     );
//     tree.update(
//         6,
//         Aabb2::new(Point2::new(0.0, 0.0, 0.0), Point2::new(512.0, 112.0, 1.0)),
//     );
//     tree.update(
//         7,
//         Aabb2::new(Point2::new(0.0, 0.0, 0.0), Point2::new(410.0, 90.0, 1.0)),
//     );
//     tree.update(
//         8,
//         Aabb2::new(
...
//         tree.add(
//             i + 1,
//             Aabb2::new(Point2::new(x, y, z), Point2::new(x, y, z)),
//             i + 1,
//         );
